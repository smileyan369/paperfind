...
SPLASH_HTML_TEMPLATE = r"""<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .logo img { width: 82px; height: 82px; border-radius: 24px; }
  @keyframes pulse {
...
def _splash_html() -> str:
    icon_path = _resource_path("app-icon.png")
    if os.path.exists(icon_path):
        with open(icon_path, "rb") as f:
            encoded = base64.b64encode(f.read()).decode("ascii")
        icon = f'<img src="data:image/png;base64,{encoded}" alt="">'
    else:
        icon = '<div style="width:82px;height:82px;border-radius:24px;background:#7e14ff"></div>'
    return SPLASH_HTML_TEMPLATE.replace("__ICON_IMAGE__", icon)
