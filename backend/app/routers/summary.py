...
async def _check_ai_available():
    ...
    if not cfg["llm_api_key"]:
        raise HTTPException(status_code=503, detail="无 AI：未配置 API Key，请先在设置中填写。")
...
async def summarize_single(request: Request, paper_id: int):
    ...
    async with async_session() as db:
        ...
        if existing and existing.status == "completed" and existing.summary_cn:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"导读生成失败: {e}") from e
    finally:
...
async def summary_stats(db: AsyncSession = Depends(get_db)):
    ...
    failed_result = await db.execute(select(func.count(Summary.id)).where(Summary.status == "failed"))
